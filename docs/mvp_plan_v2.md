...
Goal: 라운드 공개 범위, 팔로우, 지인 공개 반응, public round 검색을 연결한다.
...
- companion 정보가 있으면 라운드 상세의 `동반자 비교` 버튼이 활성화되고, companion 매칭은 동반자 이름, 동일한 코스, 동일한 시간을 기준으로 한다. 최종적으로 비교 가능한지는 해당 라운드의 공개 기준을 따른다.
