...
LalaGolf v2 MVP는 단일 사용자 중심 v1을 멀티 유저 분석 서비스로 재구성하는 첫 번째 릴리스다. MVP의 목표는 사용자가 계정을 만들고, 라운드 파일을 업로드하고, 파싱 결과를 검토한 뒤, 개인 대시보드/라운드 상세/분석 화면에서 신뢰 가능한 인사이트를 확인하고, 선택적으로 링크 공유와 구조화된 질의를 사용할 수 있게 하는 것이다.
...
- Ask LalaGolf 1차 버전: 구조화된 SQL/metric retrieval과 템플릿 답변.
...
- P3~P5 UI는 빈 mock이 아니라 실제 라운드 데이터로 검증할 수 있다.
...
10. M7 Ask LalaGolf MVP
11. M8 Migration Dry Run
12. M9 MVP Hardening

Note: M8 final diff validation still happens after M2/M5 analytics behavior is stable, but import work starts earlier so P3~P5 can be checked with real user data.
