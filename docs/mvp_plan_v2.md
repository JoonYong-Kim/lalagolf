...
- link-only 공유.
- 업로드 후 라운드 상세에서 공개 범위 설정과 public round 검색.
- follow 관계, 지인 공개 스코어카드, follow-scoped 댓글/좋아요.
- 라운드 상세의 `동반자 비교` 버튼과 접근 가능한 다른 라운드 비교 UI.
...
## M6.5 Social Visibility

Goal: 라운드 공개 범위, 팔로우, 지인 공개 반응, public round 검색, 시간순 소셜 피드를 연결한다.

Deliverables:

- 라운드 상세 visibility 설정.
- public round 검색 화면.
- public/following 라운드를 스크롤해 볼 수 있는 시간순 소셜 피드.
- follows 테이블과 팔로우 요청/수락 흐름.
- 지인 공개 라운드 댓글/좋아요.
- companion 정보가 있으면 활성화되는 라운드 상세 `동반자 비교` 진입 보조.

Acceptance Criteria:

- 사용자는 업로드 후 라운드 상세에서 공개 범위를 바꿀 수 있다.
- public 라운드는 로그인하지 않아도 검색해서 볼 수 있다.
- 비로그인 피드는 public 라운드만 보여주고, 로그인 피드는 public 라운드와 accepted follow 관계의 followers 라운드를 함께 보여준다.
- 피드는 `social_published_at desc, item_id desc` keyset pagination으로 중복/누락 없이 스크롤된다.
- accepted follow 관계가 있으면 지인 공개 라운드를 볼 수 있고, 댓글/좋아요를 남길 수 있다.
- companion 정보가 있으면 라운드 상세의 `동반자 비교` 버튼이 활성화되고, companion 매칭은 사용자가 명시적으로 저장한 companion-account 매핑, 동일한 코스, 동일한 날짜, 동일한 시간을 기준으로 한다. 최종적으로 비교 가능한지는 해당 라운드의 공개 기준을 따른다. 공개/공유 응답에는 companion 이름을 노출하지 않는다.
...
- 사용자는 연습 후 발견한 내용을 private diary로 남기고, 원하면 public 또는 followers 범위로 공개할 수 있다.
- 사용자는 라운드 목표를 private으로 만들고, 원하면 public 또는 followers 범위로 공개할 수 있다.
