...
- 향후 논의: 업로드 파일 상세도를 `Score only`, `Scorecard`, `Shot detail`로 구분하는
  공식 포맷을 검토한다. 상세도에 따라 스코어 추이/기본 비교, 퍼팅·페널티·GIR 분석,
  샷 타임라인·stroke gained·샷 품질 분석처럼 활성화 가능한 기능을 단계적으로 안내한다.
...
- 업로드 상세도별 기능 차등 지원은 추가 논의가 필요하다. 스코어만 있는 파일은 기록/추세/간단
  비교 중심, 스코어·퍼팅·페널티 파일은 스코어카드·퍼팅·페널티·추정 GIR 중심, 샷별 파일은
  현재의 stroke gained·샷 품질·인사이트·연습 계획 기능까지 활용하는 방향으로 검토한다.
