...
- v1 MySQL 데이터와 `data/<year>` 원본 라운드 파일을 v2 PostgreSQL 구조로 이전한다.
...
- Embedding generation can be delayed; analytics must not depend on it.
...
- Spot-check Ask LalaGolf with imported data.
