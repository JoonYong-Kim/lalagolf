...
`conf/lalagolf.conf` contains local credentials and web users; do not commit real secrets. Flask now reads `SECRET_KEY` from the environment and falls back to a generated development key, so keep production values in environment variables rather than source files. Sanitize sample config values in shared branches.
