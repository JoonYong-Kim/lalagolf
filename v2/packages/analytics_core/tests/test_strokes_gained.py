from lalagolf_analytics_core.expected_value import build_expected_score_table
from lalagolf_analytics_core.shot_model import normalize_shot_states
from lalagolf_analytics_core.strokes_gained import (
    summarize_approach_strategy_comparison,
    build_historical_shot_facts,
    build_shot_values,
    build_shot_values_with_fallback,
...
def test_build_shot_values_with_fallback_preserves_lookup_metadata():
    round_info = {"id": 2}
    holes = [{"holenum": 1, "par": 4}]
    current_shots = [
    ...
    current_facts = normalize_shot_states(round_info, holes, current_shots)
    key = ("tee", "160_plus", 4, "off_the_tee")
    global_table = {
        "full": {
            key: {
    ...
    valued_facts = build_shot_values_with_fallback(
        current_facts,
        [{"scope_type": "global", "table": global_table}],
    )

    assert valued_facts[0]["expected_source_scope"] == "global"
    assert valued_facts[0]["expected_confidence"] == "medium"
    assert valued_facts[0]["shot_value"] is not None
