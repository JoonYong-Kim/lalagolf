from lalagolf_analytics_core.expected_value import (
    annotate_expected_scores,
    annotate_expected_scores_with_fallback,
    build_round_recency_weights,
    build_expected_score_table,
    lookup_expected_score,
    lookup_expected_score_with_fallback,
)
...
def test_lookup_expected_score_with_fallback_prefers_global_full_over_user_coarse_fallback():
    fact = {
        "start_state": "tee",
        "distance_bucket": "160_plus",
        "par_type": 4,
    ...
    user_table = {
        "full": {
            ("tee", "160_plus", 4, "off_the_tee"): {
                "expected_strokes": 4.5,
                "sample_count": 1,
                "weighted_sample_count": 1.0,
            },
        },
        "no_par": {},
        "no_distance": {},
    ...
    result = lookup_expected_score_with_fallback(
        fact,
        [
            {"scope_type": "user", "table": user_table},
            {"scope_type": "global", "table": global_table},
        ],
        min_samples_by_level={
            "full": 3,
            "no_par": 3,
    ...
    assert result is not None
    assert result["expected_strokes"] == 3.8
    assert result["level"] == "full"
    assert result["source_scope"] == "global"
    assert result["confidence"] == "medium"


def test_lookup_expected_score_with_fallback_uses_user_when_samples_are_enough():
    fact = {
        "start_state": "fairway",
        "distance_bucket": "120_160",
        "par_type": 4,
        "shot_category": "approach",
    }
    user_table = {
        "full": {
            ("fairway", "120_160", 4, "approach"): {
                "expected_strokes": 2.7,
                "sample_count": 5,
                "weighted_sample_count": 5.0,
            },
        },
    }
    global_table = {
    ...
    result = lookup_expected_score_with_fallback(
        ...
        min_samples_by_level={"full": 3},
    )

    assert result is not None
    assert result["expected_strokes"] == 2.7
    assert result["source_scope"] == "user"
    assert result["confidence"] == "high"


def test_lookup_expected_score_with_fallback_can_use_operator_baseline():
    fact = {
        "start_state": "green",
        "distance_bucket": "0_10",
        "par_type": 4,
        "shot_category": "putting",
    }
    baseline_table = {
        "full": {},
        "no_par": {},
        ...
        "start_category": {
            ("green", "putting"): {
                "expected_strokes": 1.8,
                "sample_count": 20,
                "weighted_sample_count": 20.0,
            },
        },
    }
    ...
    result = lookup_expected_score_with_fallback(
        fact,
        [{"scope_type": "baseline", "table": baseline_table}],
        min_samples_by_level={"start_category": 2},
    )

    assert result is not None
    assert result["expected_strokes"] == 1.8
    assert result["level"] == "start_category"
    assert result["source_scope"] == "baseline"
    assert result["confidence"] == "low"


def test_annotate_expected_scores_with_fallback_exposes_lookup_metadata():
    shot_facts = _sample_shot_facts()
    fact = shot_facts[0]
    key = (
        fact["start_state"],
        fact["distance_bucket"],
        fact["par_type"],
        fact["shot_category"],
    )
    global_table = {
        "full": {
            key: {
                "expected_strokes": 3.4,
                "sample_count": 50,
                "weighted_sample_count": 50.0,
            },
        },
        "no_par": {},
        "no_distance": {},
    ...
    annotated = annotate_expected_scores_with_fallback(
        shot_facts[:1],
        [{"scope_type": "global", "table": global_table}],
    )

    assert annotated[0]["expected_before"] == 3.4
    assert annotated[0]["expected_lookup_level"] == "full"
    assert annotated[0]["expected_sample_count"] == 50
    assert annotated[0]["expected_source_scope"] == "global"
    assert annotated[0]["expected_confidence"] == "medium"
