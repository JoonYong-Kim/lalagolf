        ...
        if not header_parsed:
            ...
            if round_data['tee_off_time'] is None:
                ...
                if parsed_time:
                    ...
                    # If there's remaining content, try to parse golf course or co-players from it
                    if remaining_line:
                        parts = [p.strip() for p in remaining_line.split(' ') if p.strip()]
                        if len(parts) >= 2: # Assuming at least Golf Course and Co-players
                            round_data['golf_course'] = parts[0]
                            round_data['co_players'] = ', '.join(parts[1:])
                        elif len(parts) == 1: # Only Golf Course
                            round_data['golf_course'] = parts[0]
                else:
                    # If tee-off time not found, try to parse golf course or co-players directly
                    if round_data['golf_course'] is None and not hole_match: # Don't misinterpret hole as golf course
                        # Heuristic: if it looks like a golf course name (e.g., contains letters, not just numbers/P)
                        if not re.match(r'^\d+P\d+
...
, original_line):
                    ...
                    elif round_data['co_players'] is None and not hole_match:
                        round_data['co_players'] = original_line

            # Check if all header info is gathered or if a hole is encountered
            if round_data['tee_off_time'] and round_data['golf_course'] and round_data['co_players']:
                header_parsed = True
            elif hole_match: # If a hole is encountered, stop header parsing
                header_parsed = True

            if not header_parsed: # If still in header parsing and not a hole, add to unparsed if not already parsed
                if not (round_data['tee_off_time'] and original_line == round_data['tee_off_time']) and 
                   not (round_data['golf_course'] and original_line == round_data['golf_course']) and 
                   not (round_data['co_players'] and original_line == round_data['co_players']):
